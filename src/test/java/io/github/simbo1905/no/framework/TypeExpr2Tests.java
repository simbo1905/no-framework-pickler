...
class TypeExpr2Tests {
  ...
  void testComplexHeterogeneousListWithPickler() {
    ...
    final List<HeterogeneousItem> complexList = new ArrayList<>();
    complexList.add(new ItemString("a string"));
    complexList.add(new ItemInt(123));
    complexList.add(new ItemLong(456L));
    complexList.add(new ItemBoolean(true));
    complexList.add(new ItemNull()); // Representing null
    complexList.add(new ItemTestRecord(new RefactorTests.Person("rec1", 99)));
    complexList.add(new ItemTestEnum(TestEnum.SECOND));
    complexList.add(new ItemOptional(Optional.empty()));
    complexList.add(new ItemList(Arrays.asList("nested", "list", null))); // List with nulls included
    final var originalRecord = new ComplexListRecord(complexList);
            ...
            var componentSerdes = serdeMap.computeIfAbsent(concreteType, t ->
                Companion2.buildComponentSerdes(t, List.of(),
                    ...
                    type2 -> (buf, o) -> {
                      ...
                      } else if (type2 == TestEnum.class) {
                        TestEnum enumValue = (TestEnum) o;
        ...
        signature -> buffer -> {
          ...
          if (sealedTypes.contains(targetType)) {
            ...
            if (targetType == ItemString.class) {
              ...
              return new ItemLong((Long) componentValue);
            } else if (targetType == ItemBoolean.class) {
              return new ItemBoolean((Boolean) componentValue);
            } else if (targetType == ItemNull.class) {
              return new ItemNull();
            } else if (targetType == ItemTestRecord.class) {
              return new ItemTestRecord((RefactorTests.Person) componentValue);
            } else if (targetType == ItemTestEnum.class) {
              return new ItemTestEnum((TestEnum) componentValue);
            } else if (targetType == ItemOptional.class) {
              @SuppressWarnings("unchecked")
              Optional<String> opt = (Optional<String>) componentValue;
              return new ItemOptional(opt);
            } else if (targetType == ItemList.class) {
              @SuppressWarnings("unchecked")
